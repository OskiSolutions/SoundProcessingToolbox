%Wyznaczanie wspó³czynników MFCC (Mel-Frequency Cepstral Coefficients) dla sygna³u jednowymirowego (x)
%autor: l.laszko@ita.wat.edu.pl 
%2015
function [M,FbMfcc] = mfcc (x, Noc, Ws, Ovr, Nfft, Fs, Nof)
...
%parametry wyjœciowe:
%M      - macierz wspó³czynników MFCC
%FbMfcc - macierz banku filtrów

if(nargin<1),
    error ('Brak sygna³u do wyznaczenia wspó³czynników MFCC!');        
end
...
FbMfcc=zeros(Nof,Nfft2);

%przygotowanie macierzy czêstotliwoœci
kAid=repmat(ktSec,Nof-2,1);

%prepare the matrix of center frequences:
mAidLow=repmat(Cfreq(1:Nof-2)',1,Nfft2);
mAidMid=repmat(Cfreq(2:Nof-1)',1,Nfft2);
mAidHigh=repmat(Cfreq(3:Nof)',1,Nfft2);

%find indexes of matrix elements which fulfil the inequalities from the definition: 
...
indAidL=sub2ind(size(FbMfcc), indML, indKL);
indAidH=sub2ind(size(FbMfcc), indMH, indKH);

%za³adowanie macierzy banku filtrów
FbMfcc(indAidL)=(ktSec(indKL)'-mAidLow(indML))./(mAidMid(indML)-mAidLow(indML));
FbMfcc(indAidH)=(ktSec(indKH)'-mAidHigh(indMH))./(mAidMid(indMH)-mAidHigh(indMH));
